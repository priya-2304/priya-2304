# 👋 Hi, I'm Priya

### 💻 B.Tech CSE Student | MERN Stack Developer 

I'm a Computer Science & Engineering student passionate about building modern, responsive, and user-friendly web applications. I enjoy learning new technologies, creating real-world projects, and continuously improving my development skills.

---

## 🚀 About Me

- 🎓 B.Tech Computer Science & Engineering Student
- 💻 MERN Stack Development Enthusiast
- 🌱 Currently improving my Backend skills and learning new Technologies
- 🔥 Interested in Full Stack Web Development
- 💡 Love turning ideas into real-world applications

---

## 🛠️ Tech Stack

### Frontend
- HTML
- CSS
- JavaScript
- React.js
- Bootstrap
- Tailwind CSS

### Backend
- Node.js
- Express.js

### Database
- MongoDB
- MySQL

### Programming
- C++
- C
- Python(basic)

### Tools
- Git
- GitHub
- VS Code
- Postman

---

## 📂 Featured Projects

### 🛒 E-Commerce Website
A responsive e-commerce application built using React.js.

### 💰 SmartFee Portal
A fee management system with student login, fee dashboard, payment integration, receipts, and admin features.

### 🔐 Student Profile Card Manager
A React application demonstrating reusable components, props, state, and dynamic student cards.

---

## 📜 Certifications & Achievements

- Cisco Introduction to Cybersecurity
- Smart India Hackathon Participant
- Code Relay – 3rd Position
---

## 📫 Connect With Me

- 💼 LinkedIn - https://www.linkedin.com/in/priya-deswal423/
- 📧 Email - deswal9485@gmail.com

---

⭐ Thanks for visiting my profile!
